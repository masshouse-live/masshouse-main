...
@section('content')
    <main class="flex flex-col h-screen space-y-6 ">
        ...
        <div class="flex w-full">
            <div class="grid grid-cols-4 xl:grid-cols-5 p-4 gap-2 w-full" id="sortable">
                @foreach ($sponsors as $sponsor)
                    <div class="aspect-video w-full border col-span-1 cursor-move relative overflow-hidden"
                        data-id="{{ $sponsor->id }}" data-from="{{ $sponsor->rank }}">
                        ...
                        <img src="{{ asset($sponsor->logo_white) }}"
                            class="aspect-video h-full object-cover cursor-move overflow-hidden" />
                    </div>
                @endforeach
            </div>
        </div>
